...
class NIDMUpload:
    ...
    def parse_metafiles(self, extract_ttl=False):
        ...
        for ext in [".ttl"]:
            ...
            if not metafiles[ext]:
                raise self.ParseException("No {0} file found in zip.".format(ext))

        self.ttl = metafiles[".ttl"][0]
        # fix incorrect property format in earlier versions of SPM12 output
        self.raw_ttl = self.fix_spm12_ttl(
            self.zip.read(metafiles[".ttl"][0]).decode("utf-8")
        )
    ...
    def parse_statmap_type(stattype_url):
        stato_dict = {
            "http://purl.obolibrary.org/obo/STATO_0000376": "Z",
