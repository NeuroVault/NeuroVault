    ...
    def testThresholded(self):
        for p,t in zip(self.ratios, self.thresholded):
            ...
            if p != 0.0:
                ...
                random_idx = np.random.choice(list(range(empty_data.size)), number_voxels, replace=False)
                empty_data[np.unravel_index(random_idx, empty_data.shape)] = 0
            empty_nii = nb.Nifti1Image(empty_data,affine=self.brain.affine,header=self.brain.get_header())
