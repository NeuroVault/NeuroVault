...
class Test_atlas_queries(TestCase):
    ...
    def test_query_region_out_of_order_indices(self):
        ...
        for region in atlasRegions:
            orderedURL = 'http://127.0.0.1:8000/api/atlases/atlas_query_region/?region=%s&atlas=orderedAtlas&collection=atlasCollection' %region
            orderedResponse = self.client.get(orderedURL, follow=True)
            unorderedURL = 'http://127.0.0.1:8000/api/atlases/atlas_query_region/?region=%s&atlas=unorderedAtlas&collection=atlasCollection' %region
        ...
        for region, testVoxels in testRegions.items():
            URL = 'http://127.0.0.1:8000/api/atlases/atlas_query_region/?region=%s&atlas=orderedAtlas&collection=atlasCollection' %region
    ...
    def test_out_of_order_indices(self):
        ...
        for region, testVoxels in testRegions.items():
            for triple in testVoxels:
                ...
                URL = 'http://127.0.0.1:8000/api/atlases/atlas_query_voxel/?x=%s&y=%s&z=%s&atlas=orderedAtlas&collection=atlasCollection' % (X, Y, Z)
                response = self.client.get(URL, follow=True)
