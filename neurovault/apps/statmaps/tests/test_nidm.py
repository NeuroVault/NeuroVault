...
NIDM_TEST_FILES = OrderedDict(
    {
        "spm_ds005_sub-01": {
            "file": os.path.join(TEST_PATH, "test_data/nidm/spm_ds005_sub-01.nidm.zip"),
            "output_row": {"type": "T", "name": "Statistic Map: neg loss param"},
            "num_statmaps": 1,
        },
        "spm_ds005_group": {
            "file": os.path.join(TEST_PATH, "test_data/nidm/spm_ds005_group.nidm.zip"),
            "output_row": {"type": "T", "name": "Statistic Map: neg loss param"},
            "num_statmaps": 1,
        },
        "fsl_ds005_sub-01": {
            "file": os.path.join(TEST_PATH, "test_data/nidm/fsl_ds005_sub-01.nidm.zip"),
            "output_row": {"type": "T", "name": "Statistic Map: neg_loss_param"},
            "num_statmaps": 2,
        },
        "fsl_ds005_group": {
            "file": os.path.join(TEST_PATH, "test_data/nidm/fsl_ds005_group.nidm.zip"),
            "output_row": {"type": "T", "name": "Statistic Map: group mean"},
            "num_statmaps": 2,
        },
        "fsl_course_av": {
            "file": os.path.join(TEST_PATH, "test_data/nidm/fsl_course_av.nidm.zip"),
            "output_row": {"type": "T", "name": "Statistic Map: Visual"},
            "num_statmaps": 4,
        },
        "fsl_course_fluency2": {
            "file": os.path.join(
                TEST_PATH, "test_data/nidm/fsl_course_fluency2.nidm.zip"
            ),
            "output_row": {"type": "F", "name": "Statistic Map: Generation F"},
            "num_statmaps": 12,
        },
        "spm_example": {
            "file": os.path.join(TEST_PATH, "test_data/nidm/spm_example.nidm.zip"),
            "output_row": {
                "type": "T",
        ...
        "spm_auditory_v1.2.0": {
            "file": os.path.join(TEST_PATH, "test_data/nidm/auditory.nidm.zip"),
            "output_row": {
                "type": "T",
                "name": "Statistic Map: passive listening > rest",
            },
        ...
        "fsl_course_ptt_ac_left": {
            "file": os.path.join(
                TEST_PATH, "test_data/nidm/fsl_course_ptt_ac_left.nidm.zip"
            ),
            "output_row": {"type": "F", "name": "Statistic Map: index F"},
            "num_statmaps": 12,
        },
    }
)
...
class NIDMResultsTest(TestCase):
    def setUp(self):
        ...
        self.failing_files = {
            "spm_bad_ttl": os.path.join(
                TEST_PATH, "test_data/nidm/spm_bad_ttl.nidm.zip"
            )
        }
    ...
    def testParseNIDMZip(self):
        ...
        for name, info in list(self.files.items()):
            ...
            for field in "name", "type":
                self.assertTrue(
                    info["output_row"][field] in [a[field] for a in statmaps[name]]
                )
    ...
    def testUploadNIDMZip(self):
        for name, info in list(self.files.items()):
            ...
            self.assertEqual(
                len(nidm.nidmresultstatisticmap_set.all()), info["num_statmaps"]
            )

            map_type = info["output_row"]["type"][0]
            map_imgs = nidm.nidmresultstatisticmap_set.filter(map_type=map_type).all()
    ...
    def testDownloadCollection_NIDM_results(self):
        ...
        image = save_statmap_form(
            image_path=os.path.join(TEST_PATH, "test_data/statmaps/all.nii.gz"),
        ...
        self.client.login(username=self.user)
        request = factory.get(
            "/collections/%s/download" % collection.pk, {"format": "img.zip"}
        )
