...
class Study(models.Model):
    ...
    ''' I commented all the fields out because many of them are blank = false, which means they are 
    Required, and the form won't validate if the user doesn't pass them. We can't really force users
    To fill them all out, so we need to either set blank = true for everything except name/description
    (which is what I've done below), or make sure that the form fields always have defaults that 
    get set even if the user doesn't do anything.
    -- Tal
    '''
    ...
    type_of_design = models.CharField(choices=[('blocked', 'blocked'), ('eventrelated', 'event_related'), ('hybridblockevent', 'hybrid block/event'), ('other', 'other')], max_length=200, blank=True, help_text="Blocked, event-related, hybrid, or other", null=False, verbose_name="ExperimentalDesign.type_of_design")
    number_of_imaging_runs = models.IntegerField(help_text="Number of imaging runs acquired", null=True, verbose_name="ExperimentalDesign.number_of_imaging_runs", blank=True)
    number_of_experimental_units = models.IntegerField(help_text="Number of blocks, trials or experimental units per imaging run", null=True, verbose_name="ExperimentalDesign.number_of_experimental_units", blank=True)
    length_of_runs = models.FloatField(help_text="Length of each imaging run in seconds", null=True, verbose_name="ExperimentalDesign.length_of_runs", blank=True)
    length_of_blocks = models.FloatField(help_text="For blocked designs, length of blocks in seconds", null=True, verbose_name="ExperimentalDesign.length_of_blocks", blank=True)
