...
class TestAtlas(APITestCase):
    ...
    def test_query_region_out_of_order_indices(self):
        ...
        for region in atlasRegions:
            orderedURL = (
                "http://127.0.0.1:8000/api/atlases/atlas_query_region/?region=%s&atlas=orderedAtlas&collection=Collection1"
                % (region)
            )
            orderedResponse = self.client.get(orderedURL, follow=True)
            unorderedURL = (
                "http://127.0.0.1:8000/api/atlases/atlas_query_region/?region=%s&atlas=unorderedAtlas&collection=Collection1"
                % (region)
            )
            ...
            orderedTriples = [
                (orderedList[0][i], orderedList[1][i], orderedList[2][i])
                for i in range(len(orderedList[0]))
            ]
            unorderedTriples = [
                (unorderedList[0][i], unorderedList[1][i], unorderedList[2][i])
                for i in range(len(unorderedList[0]))
            ]
            orderedSorted = sorted(orderedTriples, key=itemgetter(0))
            unorderedSorted = sorted(unorderedTriples, key=itemgetter(0))
            print(
                "Equality of lists for region %s: %s"
                % (region, orderedSorted == unorderedSorted)
            )
            self.assertEqual(orderedSorted, unorderedSorted)

        print("\nAssessing consistency of results for regional query...")
        testRegions = {
            "6v": [(58.00, -4.00, 18.00), (62.00, 6.00, 30.00)],
        ...
        for region, testVoxels in list(testRegions.items()):
            ...
            triples = [
                (voxelList[0][i], voxelList[1][i], voxelList[2][i])
    ...
    def test_out_of_order_indices(self):
        ...
        for region, testVoxels in list(testRegions.items()):
            for triple in testVoxels:
                ...
                URL = (
                    "http://127.0.0.1:8000/api/atlases/atlas_query_voxel/?x=%s&y=%s&z=%s&atlas=orderedAtlas&collection=Collection1"
                    % (X, Y, Z)
                )
                response = self.client.get(URL, follow=True)
    ...
    def test_atlases_datatable(self):
        ...
        self.assertEqual(
            data["label_description_file"].split("/")[-1],
