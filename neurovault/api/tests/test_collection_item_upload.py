...
class TestCollectionItemUpload(APITestCase):
    ...
    def test_upload_statmap(self):
        ...
        self.assertEqual(
            response.data["cognitive_paradigm_cogatlas"], "action observation task"
        )
        ...
        self.assertEqual(
            statmap.cognitive_paradigm_cogatlas.name, "action observation task"
        )
        self.assertEqual(
            statmap.cognitive_contrast_cogatlas.name,
    ...
    def test_upload_statmap_with_metadata(self):
        ...
        statmap = StatisticMap.objects.get(pk=response.data["id"])
        self.assertEqual(statmap.map_type, "T")
        self.assertEqual(statmap.data["custom_metadata_numeric_field"], "42")
        self.assertEqual(statmap.data["custom_metadata_string_field"], "forty two")
    ...
    def test_upload_atlas(self):
        ...
        self.assertEqual(response.data["collection_id"], self.coll.id)
        self.assertRegex(response.data["file"], r"\.nii\.gz$")
        self.assertRegex(response.data["label_description_file"], r"\.xml$")
    ...
    def _test_upload_nidm_results(self, url, name, data):
        ...
        post_dict = {
            ...
            "zip_file": SimpleUploadedFile(fname, open(data["file"], "rb").read()),
        }
        ...
        self.assertEqual(
            len(nidm.nidmresultstatisticmap_set.all()), data["num_statmaps"]
        )
    ...
    def test_missing_required_fields(self):
        ...
        expect_dict = {
            "map_type": ["This field is required."],
